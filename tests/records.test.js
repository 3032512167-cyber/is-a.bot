...
const validRecordTypes = new Set(["A", "AAAA", "CAA", "CNAME", "DS", "MX", "NS", "SRV", "TLSA", "TXT"]);
        ...
        if (["CAA", "DS", "SRV", "TLSA"].includes(key)) {
            ...
            value.forEach((record, idx) => {
                ...
                if (key === "CAA") {
                    ...
                    t.true(
                        Number.isInteger(record.matching_type) &&
                            record.matching_type >= 0 &&
                            record.matching_type <= 255,
...
t("Root subdomains should have at least one usable record", (t) => {
    const usableRecordTypes = ["A", "AAAA", "CNAME", "MX", "NS"];
    ...
    files.forEach((file) => {
        ...
        t.true(
            ...
            `${file}: Root subdomains must have at least one A, AAAA, CNAME, MX, or NS record`
        );
    });
});
