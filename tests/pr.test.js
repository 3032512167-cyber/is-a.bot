...
t("Users can only update their own subdomains", (t) => {
    if (requiredEnvVars.every((v) => process.env[v])) {
        ...
        changedJSONFiles.forEach((file) => {
            ...
            if (data.owner.username === "free-domains") {
                t.true(
                    admins.includes(prAuthorId),
                    `${subdomain}: ${prAuthor} is not authorized to update ${subdomain}.is-a.bot`
                );
            } else {
                t.true(
                    data.owner.username.toLowerCase() === prAuthor || trusted.includes(prAuthorId),
                    `${file}: ${prAuthor} is not authorized to update ${subdomain}.is-a.bot`
                );
            }
        });
        ...
        deletedJSONFiles.forEach((file) => {
            ...
            if (data.owner.username === "free-domains") {
                t.true(
                    admins.includes(prAuthorId),
                    `${subdomain}: ${prAuthor} is not authorized to delete ${subdomain}.is-a.bot`
                );
            } else {
                t.true(
                    data.owner.username.toLowerCase() === prAuthor || trusted.includes(prAuthorId),
                    `${file}: ${prAuthor} is not authorized to delete ${subdomain}.is-a.bot`
                );
            }
        });
    }
